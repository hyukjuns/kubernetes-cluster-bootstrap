...
Vagrant를 사용해 Kubernetes의 노드를 생성하고, <br>
kubeadm을 사용해 Kubernetes Cluster를 Bootstrap 하는 Hands-On Tutorial 자료 입니다.

>Kubernetes 공식 문서 기반으로 진행하였으며, 진행중 겪은 에러를 기반으로 동일한 에러를 겪지 않도록 수정하며 작성하였습니다.
...
## Prerequire
1. 맥북 초기 셋업 및 클러스터 관리 환경 세팅
    > 저는 macbook을 초기화 하고 시작하여 brew부터 설치했습니다. 이미 환경이 마련되신 분들은 적절한 Tool을 선택적으로 설치하시면 됩니다.
    - Installed Tools 
        - brew
        - ohmyzsh (+theme)
        - vscode (+k8s plugin
...
1. hostname 변경: 각 노드는 hostname이 고유해야 합니다. 저는 worker 노드의 hostname만 변경 하였습니다.

2. Off SELinux
    > kubeadm은 아직 selinux환경을 지원하지 않고 있다. 자세하게는 container가 pod의 Network에 도달해야 하기 위해서는 selinux off가 필요합니다.
...
3. Off Swap
    ...
    - fstab swapoff
        ...
        ```
4. Off Firewalld
    > 방화벽에서 필수 포트만 개별적으로 개방해도 되지만, 본 환경은 테스트 목적으로 OS수준의 방화벽을 내렸습니다.
    ```
    sudo systemctl stop frewalld
    sudo systemctl disable firewalld
    ```
5. Setup iptables config
    > 클러스터의 NetworkPlugin의 요구사항으로 NetworkPlugin이 올바르게 동작하기 위한 설정이다, kube-proxy는 리눅스의 iptables 기반이며, 클러스터에 구성된 브릿지 트래픽은 iptables에 의해 올바르게 처리되어야 하기 위한 설정입니다.
...
6. Install Container Runtime
    > 쿠버네티스는 파드가 노드에서 실핼될 수 있도록 클러스터 각 노드에 컨테이너 런타임을 설치해야 합니다. 파드는 설치된 컨테이너 런타임을 사용해 컨테이너를 실행 합니다. 쿠버네티스는 CRI(kubelet이 Container와 통신하기 위해 사용하는 API로 이해할 수 있다.)를 사용하여 사용자가 선택한 컨테이너 런타임과 상호작용 하기 때문에, 쿠버네티스의 CRI 요구사항을 만족하는 컨테이너 런타임을 사용 해야합니다.
    ...
    - Install Docker Runtime
        ...
        ```
7. cgroup 드라이버 설정
    > Continaer Runtime과 kubelet의 cgroup 드라이버를 일치시켜야 한다. kubelet은 systemd 드라이버를 사용하므로, docker cgroup driver를 cgroupfs에서 systemd로 변경해야 합니다. (CNCF에서는 cgroupfs보다 systemd드라이버를 권장)
...
8. kubeadm, kubelet, kubectl 설치
    ...
    2. 패키지 설치 및 kubelet 활성
        ...
        ```
        > kubelet 활성 이후 "systemctl status kubelet으로 화인시 status가 failure인 이유는 아직 kubeadm으로 bootstrap 전 이므로 kubelet의 status는 비정상으로 확인되며 이는 정상입니다.
...
1. Bootstrap Control Plane
    ...
    > 자세한 init workflow는 아래 링크에서 확인할 수 있습니다.
    https://kubernetes.io/docs/reference/setup-tools/kubeadm/kubeadm-init/#init-workflow

    - 출력
        > 제 환경에서 생성된 토큰 값을 가리기 위해 TOKEN_VALUE, TOKEN_HASH_VALUE 로 치환해두었습니다. 참고해주세요.
        ```
        ...
        kubeadm join 192.168.0.20:6443 --token <TOKEN_VALUE> \
                --discovery-token-ca-cert-hash <TOKEN_HASH_VALUE> 
        ```
2. kubernetes의 config(kubeconfig)을 사용하기 위한 설정
     ```
    mkdir -p $HOME/.kube
    sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
    sudo chown $(id -u):$(id -g) $HOME/.kube/config
    ```
    > kubeadm을 사용하여 클러스터 설치 시 기본적으로 admin.conf가 생성되고 이를 통해 쿠버네티스 api서버와 통신하게 된다. 해당 conifg을 비 루트 유저(본 가이드에선 현재 작업중인 리눅스 사용자 ex: vagrant)도 사용할 수 있도록 설정하는 작업이다. 해당 파일을 마스터 노드 뿐 아니라 원격 환경으로 가져와서 원격 환경에서 클러스터 API와 통신할 수 있다. 다만, 주의해야 할 점은 해당 config은 admin 권한으로 클러스터에 대한 모든 권한을 행사할 수 있다. 따라서 별도의 user를 생성하고 그에 알맞는 권한이 부여된 config을 새로 생성하여 사용하는것이 안전하다. 관련 명령어로 "kubeadm kubeconfig user"을 통해 config을 생성할 수 있다.
    
    - kubeconfig 파일을 Host PC 로 복사하여 Host PC에서 클러스터를 관리합니다.
        > vagrant로 가상머신을 생성했기에, vagrant-scp 플러그인을 통해 MAster 노드에 있는 파일을 호스트 머신으로 copy합니다. vagrant가 아닐경우, 일반적인 scp 를 사용해서 파일을 로컬 환경으로 복사 하실 수 있습니다.
        - 설치
            ```
        ...
        - 파일 복사 (vagrant scp src dest)
            ...
            ```
    - kubeconfig 사용 확인
        > config이 정상적으로 세팅되었는지, 그리고 현재 노드의 상태, System Pod의 상태도 확인해 봅니다.
...
3. Network Plugin (CNI) 설치
    > 클러스터의 모든 Pod가 상호 통신할 수 있도록 하는 Network Plugin을 설치하자, 기본적으로 제공되는 kubenet Plugin은 다른 노드의 pod와 통신을 지원하지 못한다. 따라서 calico와 같은 NetworkPlugin을 별도로 설치해야 한다.
    - calico CNI 설치
        ```
...
- 컨트롤플레인 작업 이후 클러스터에 등록할 워커노드 작업을 수행합니다.
- kubeadm은 관리자 권한이 필요하므로, "sudo su -" 로 root 로 변경하거나 sudo를 붙힙니다.
- 또한 마스터노드와 워커노드의 hostname이 동일하면 안되며, 현재 테스트 환경에선 worker 노드의 호스트네임을 dns resolve하지 못하기 때문에 worker node에서 join 전 "/etc/hosts"에 자신의 ipv4 ip를 자신의 hostname과 맵핑해주어야 합니다

1. 앞서 모든 노드에서 해야할 작업을 수행합니다. [all node]
...
- 출력
    ...
    * The Kubelet was informed of the new secure connection details.
    ...
    ```
- 현재 kubeconfig을 사용할 수 있는 master node에서 join된 worker 노드를 확인합니다.
...
## Clean Up
클러스터를 삭제하고 가상머신도 삭제합니다.

1. 클러스터 제거
    > Worker노드부터 진햅합니다. Worker노드는 3번까지 진행하여 제거하고, Master노드는 1번의 kubeadm reset시 클러스터에서 제거됩니다. root 권한으로 진행합니다. (sudo su -)
    1. node reset
        ```
    ...
    3. worker 노드는 위 내용을 수행한 뒤 아래 커맨드로 노드를 제거합니다.
        ...
        ```
2. 가상머신 제거
    > vagrant command로 VBOX에서 호스팅한 가상머신을 삭제합니다.
    ```
